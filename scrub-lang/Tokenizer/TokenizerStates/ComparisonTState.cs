...
public class ComparisonTState(Tokenizer context) : TokenizerStateBase(context)
{
	...
	public override void Consume(char c, int line, int col)
	{
		...
		{
			context.AddToken(new Token(TokenType.And, firstChar.ToString() + secondChar.ToString(), line, firstCol));
			context.ExitState(this);
			return;
		}
		...
		if (firstChar == '|' && secondChar == '|')
		{
			context.AddToken(new Token(TokenType.Or, firstChar.ToString() + secondChar.ToString(), line, firstCol));
			context.ExitState(this);
			return;
		}
		...
		if (firstChar == '|' && secondChar != '|')
		{
			context.AddToken(new Token(TokenType.BitwiseOr, firstChar.ToString(), line, firstCol));
			context.ExitState(this);
			context.ConsumeNext(c,line,col);
			return;
		}
